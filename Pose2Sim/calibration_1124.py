...
def calibrate_extrinsics(calib_dir, extrinsics_config_dict, C, S, K, D):
    ...
    if extrinsics_method in {'board', 'scene'}:
        ...
        # Lists to store data for bundle adjustment
        all_camera_params = []
        all_points_2d = []
        all_camera_indices = []
        all_point_indices = []
        ...
        for i, cam in enumerate(extrinsics_cam_listdirs_names):
            ...
            if extrinsics_method == 'board':
                ...
                #     raise ValueError('No points clicked (or fewer than 6). Press \'C\' when the image is displayed, and then click on the image points corresponding to the \'object_coords_3d\' you measured and wrote down in the Config.toml file.')
                                # Fixed 2D points for each camera
                fixed_points = {
                    'ext_cam01_img': np.array([
                        [[1594.2499, 1557.4208]],
                        [[1703.8025, 1606.2313]],
                        [[1844.8107, 1567.1829]],
                        [[1728.7501, 1524.8804]],
                ...
                # Get camera name from current directory
                cam_name = os.path.basename(cam)
                
                # Use fixed points instead of interactive selection
                imgp = fixed_points[cam_name]
                objp = object_coords_3d
            ...
            points_2d = imgp.reshape(-1, 2)
            all_points_2d.append(points_2d)
            camera_indices = np.full(len(points_2d), i, dtype=int)
            all_camera_indices.append(camera_indices)
            point_indices = np.arange(len(points_2d))
            all_point_indices.append(point_indices)
            all_camera_matrices.append(mtx)
            ...
            #     visualize_reprojection(img_vid_files[0], imgp, proj_obj, "Before Bundle Adjustment")
        ...
        use_bundle = False
        if extrinsics_method == 'board' and extrinsics_config_dict.get('board', {}).get('bundle_adjustment', False):
            use_bundle = True
        elif extrinsics_method == 'scene' and extrinsics_config_dict.get('scene', {}).get('bundle_adjustment', False):
            use_bundle = True
            
        if use_bundle:
            print("\nPreparing for global bundle adjustment:")
            print(f"Number of cameras processed: {len(all_camera_params)}")
            
            # Convert lists to arrays
            all_camera_params = np.array(all_camera_params)
            all_points_2d = np.concatenate(all_points_2d)
            all_camera_indices = np.concatenate(all_camera_indices)
            all_point_indices = np.concatenate(all_point_indices)
            ...
            for cam_idx in range(len(extrinsics_cam_listdirs_names)):
                ...
                f_opt = camera_params_opt[cam_idx, 6]  # Get optimized focal length
                
                # Update K matrix with optimized focal length
                K[cam_idx][0, 0] = f_opt  # fx
                K[cam_idx][1, 1] = f_opt  # fy
                ...
                if show_reprojection_error:
                    ...
                    # Calculate error
                    error = np.sqrt(np.mean((proj_obj_opt - cam_points_2d.reshape(-1, 2))**2))
                    ret.append(error)
...
def visualize_reprojection(img_path, imgp, proj_obj, title_prefix=""):
    """
    Visualize reprojection results on an image
    
    Args:
        img_path: Path to the image file
        imgp: Original image points
        proj_obj: Projected 3D points
        title_prefix: Prefix for the window title (e.g., "Before BA" or "After BA")
    """
    ...
    im_pil.show(title = f"{title_prefix} {os.path.basename(img_path)}")
